...
resource "aws_vpc" "main" {
  ...
  tags = {
    Name        = "prod-lab-vpc-${local.suffix}"
    Environment = "prod"  
    ManagedBy   = "Terraform"
  }
}
...
resource "aws_subnet" "public_a" {
  ...
  tags = {
    Name        = "prod-public-a-${local.suffix}"
  }
}
...
resource "aws_subnet" "public_b" {
  ...
  tags = {
    Name        = "prod-public-b-${local.suffix}" 
    Environment = "prod"
  }
}
...
resource "aws_route_table" "public_rt" {
  ...
  tags = {
    Name        = "prod-public-rt-${local.suffix}"
    Environment = "prod"
  }
}
...
# ALB
resource "aws_lb" "alb" {
  name               = "prod-lab-alb-${local.suffix}"
...
resource "aws_lb_target_group" "tg" {
  ...
  name     = "prod-${each.key}-tg-${local.suffix}"
...
resource "aws_launch_template" "lt" {
  for_each               = local.services
  name_prefix            = "prod-${each.key}-lt-${local.suffix}-"
  image_id               = var.ami_id
...
EOF
  ...
  tag_specifications {
    ...
    tags = {
      Name        = "prod-${each.key}-instance-${local.suffix}" 
      Service     = each.key
      Environment = "prod"  
    }
  }
...
resource "aws_autoscaling_group" "asg" {
  for_each             = local.services
  name                 = "prod-${each.key}-asg-${local.suffix}"
  min_size             = 1
  ...
  tag {
    key                 = "Name"
    value               = "prod-${each.key}-asg-${local.suffix}"
